...
export function musicWinContent(winParent: GameObj) {
	...
	timeText = winParent.add([
		...
		anchor("center"),
		{
			...
			update() {
				...
				let theText = `${formatTime(musicHandler.currentTime, false)}/${
					formatTime(musicHandler.totalTime === undefined ? musicHandler.duration() : musicHandler.totalTime, false)
				}`;
				this.text = theText;
			},
		},
	...
	get("musicButton", { recursive: true }).forEach(mBtn =>
		mBtn.onClick(() => {
